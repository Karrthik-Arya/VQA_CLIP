...
class VQAv2Dataset(Dataset):
    ...
    def __init__(self,root,split,dataset,transform):
        ...
        if dataset == 'VG':
            print(self.df["image_path"][-2:]) 
            print(self.df["question"][-2:]) 
            print(self.df["answers"][-2:])        
    ...
    def __getitem__(self, index):
        ...
        # question_feature = model.encode_text(inputs).to(torch.float32)
        # features = model.encode_image(image).to(torch.float32)
        # features = self.images[index]
        # question_feature = self.questions[index]
